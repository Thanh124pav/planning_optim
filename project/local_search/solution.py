...
INITIAL_REPAIR_FILL_ROUNDS = int(os.environ.get("LS_ORIG_INITIAL_REPAIR_FILL_ROUNDS", "2"))
MAX_LOCAL_SEARCH_ROUNDS = int(os.environ.get("LS_ORIG_MAX_ROUNDS", "5"))
...
def solve():
    ...
    # Local search: repeatedly insert currently unselected fields into the best
    # feasible day, then repair days whose load is below the factory minimum.
    improved = True
    rounds = 0
    while improved and rounds < MAX_LOCAL_SEARCH_ROUNDS:
        improved = False
        rounds += 1
        for f in sorted(fields, key=lambda x: -x[1]):
            i, d, _, _ = f
            if assign[i] > 0:
                continue
            day = best_day(f, loads, cap)
